...
A python library that uses browser automation to login to youtube & like youtube videos automatically
...
```sh
pip install youtube-video-auto-like
```

### Import

```javascript
from youtube-video-auto-like import *
```
...
To like a video we use ``auto_like`` function
It requires **video_url** as input parameter

```javascript
youtube.auto_like(video_url='video_url_here')
```
